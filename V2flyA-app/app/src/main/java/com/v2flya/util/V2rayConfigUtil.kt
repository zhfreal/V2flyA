...
import com.v2flya.dto.AngConfig.VmessBean
import com.v2flya.dto.V2rayConfig
...
object V2rayConfigUtil {
    ...
    fun getV2rayConfig(app: AngApplication, vmess: VmessBean): Result {
        ...
        return try {
            result = if (vmess.configType == EConfigType.CUSTOM.value) {
                getV2rayConfigType2(app, vmess)
            } else {
                getV2rayConfigType1(app, vmess)
            }
                ...
                if (vnext != null) {
                    for (i in 0 until vnext.length()) {
                        ...
                        return if(Utils.isIpv6Address(address)) {
                            Pair(String.format("[%s]:%s", address, port), tag)
                        } else {
                            Pair(String.format("%s:%s", address, port), tag)
                        }
                    }
